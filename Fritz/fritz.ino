// PROJECT 3 Interactive Traffic Light
 ...
 void setup() { //configure all LEDs as
 output pinMode(carRed, OUTPUT);
...
digitalWrite(carGreen, HIGH); //initialize green traffic light on
 ...
 void loop() 
{ int state = digitalRead(button); // test if the button is pressed and if 5 seconds have passed after it is pressed lately.
...
delay(500); digitalWrite(carRed, LOW); //red traffic light off 
digitalWrite(carYellow, HIGH); //yellow traffic light on 
delay(1000); 
delay(1000); digitalWrite(carYellow, LOW); //yellow traffic light off 
digitalWrite(carGreen, HIGH); //green traffic light on 

changeTime = millis() // record the duration since last change //back to the loop of main code 
}
